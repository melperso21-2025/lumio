...
### `sale_items`
Líneas de detalle de cada venta. Cada fila representa un producto dentro de una venta.
| Columna | Tipo | Notas |
|---|---|---|
| id | uuid | PK |
| company_id | uuid | FK → companies |
...
| unit_price | numeric | Precio al momento de la venta — no cambia si el producto cambia de precio |
| unit_cost | numeric | Costo al momento de la venta — capturado desde products.unit_cost |
| discount_amount | numeric | Descuento aplicado a esta línea específica |
...
> **Triggers activos:**
> - `tg_sync_inventory_from_sale_item` — al INSERT genera `inventory_movements` con `type='out'`, `reason='sale'`. Al soft delete (UPDATE con deleted_at) genera `inventory_movements` con `type='in'`, `reason='return'`
> - `tg_recalculate_sale_totals` — recalcula automáticamente en `sales`: `gross_total`, `discount_amount`, `production_cost` (SUM quantity×unit_cost), y `lines_per_order` (COUNT de items activos)
> `tg_sync_inventory_from_sale_item` | sale_items | INSERT/UPDATE | AFTER | sync_inventory_from_sale_item |
> `tg_recalculate_sale_totals` | sale_items | INSERT/UPDATE/DELETE | AFTER | recalculate_sale_totals |

> **Regla crítica:** Nunca insertar manualmente en `inventory_movements` para ventas — el trigger lo hace automáticamente desde `sale_items`. Hacerlo manualmente produce duplicados de stock.

> **Constraints:**
> - `quantity` CHECK > 0
> - `subtotal` es GENERATED ALWAYS — nunca insertarlo manualmente
...
### Ventas
- `/sales` — listado con KPIs, filtros y tabla (`SalesOverview`)
- `/sales/[id]` — detalle de venta con líneas, totales y movimientos de inventario

### API Routes
- `GET /api/sales/[id]` — retorna venta con `sale_items` y productos
- `PATCH /api/sales/[id]` — edita venta con lógica de reverso de stock

### Componentes
- `QuickSaleForm` — formulario POS de nueva venta (modal)
- `EditSaleModal` — modal de edición de venta existente
- `SalesHistoryTable` — tabla con filtros, paginación, navegación y acciones
