-- Fix flujo 5: Ad campaigns / weekly snapshots
--
-- Bugs corregidos:
--   S1: calculate_weekly_snapshot consultaba bank_transactions sin
--       deleted_at IS NULL → transacciones soft-deleted distorsionaban
--       cash_days, net_margin_pct y fixed_vs_total_pct.
--   S2: v_overdue_receivables incluía CxC ya pagadas o canceladas con
--       due_date pasado → saldo vencido inflado.
--   R1: campaigns_select sin deleted_at IS NULL → campañas eliminadas
--       visibles en acceso directo a la tabla.
--   R2: campaigns_update sin deleted_at IS NULL → registros eliminados
--       podían editarse.

-- ── S1 + S2. Recrear calculate_weekly_snapshot con los tres fixes ────────────

DROP FUNCTION IF EXISTS public.calculate_weekly_snapshot(uuid, integer, integer);
...
BEGIN
  ...
  -- Determinar rango de fechas de la semana
  SELECT sub.d_min, sub.d_max
  INTO v_week_start, v_week_end
  FROM (
    SELECT
      (SELECT MIN(x) FROM (
        SELECT sale_date::date AS x FROM public.sales
        WHERE company_id = p_company_id AND year = p_year AND week_number = p_week_number AND deleted_at IS NULL
        UNION ALL
        SELECT campaign_date::date AS x FROM public.ad_campaigns
        WHERE company_id = p_company_id AND year = p_year AND week_number = p_week_number AND deleted_at IS NULL
      ) u) AS d_min,
      (SELECT MAX(x) FROM (
        SELECT sale_date::date AS x FROM public.sales
  ...
  SELECT
    ...
    CASE
      ...
      ELSE (SUM(s.gross_total - COALESCE(s.production_cost, 0))::numeric
            / NULLIF(SUM(s.gross_total), 0)) * 100
    END
  ...
  -- Saldo bancario (cuentas activas)
  ...
  -- S1 FIX: agregar deleted_at IS NULL a ambas queries de bank_transactions
  ...
  -- S2 FIX: excluir CxC pagadas/canceladas; status='overdue' nunca se almacena
  -- por lo que la condición correcta es solo due_date < CURRENT_DATE con status activo
  ...
  WHERE ar.company_id = p_company_id
    ...
    AND ar.due_date::date < CURRENT_DATE;

  INSERT INTO public.weekly_snapshots (
    id, company_id, year, week_number,
    total_sales, total_transactions, avg_lpp, total_discounts, gross_margin_pct,
    total_ad_spend, avg_roas, total_leads, avg_effectiveness, avg_ctr,
    inventory_days, cash_days, net_margin_pct, fixed_vs_total_pct,
    overdue_receivables, avg_ticket, frozen_capital,
    created_at, updated_at
  ) VALUES (
    gen_random_uuid(), p_company_id, p_year, p_week_number,
    v_total_sales, v_total_transactions, v_avg_lpp, v_total_discounts, v_gross_margin_pct,
    v_total_ad_spend, v_avg_roas, v_total_leads, v_avg_effectiveness, v_avg_ctr,
    v_inventory_days, v_cash_days, v_net_margin_pct, v_fixed_vs_total_pct,
    v_overdue_receivables, v_avg_ticket, v_frozen_capital,
    now(), now()
  )
...
-- ── R1 + R2. Recrear políticas RLS de ad_campaigns con deleted_at IS NULL ────
...
CREATE POLICY campaigns_select ON public.ad_campaigns
  ...
  USING (
    deleted_at IS NULL
    AND (company_id = get_user_company_id() OR is_pulse_admin())
  );

DROP POLICY IF EXISTS campaigns_update ON public.ad_campaigns;
CREATE POLICY campaigns_update ON public.ad_campaigns
  FOR UPDATE
  USING (
    deleted_at IS NULL
